...
主畫面是三條等寬頻道。左側 `dashboard-column-live` 固定放自動更新模組：氣象、加權指數、新冠、流感與台彩；清單由 `app.js` 的 `AUTO_UPDATED_MODULE_IDS` 明確管理。其他模組進入右側兩條頻道，每張卡渲染後比較兩欄實際高度，放進當下較短的一欄，形成由上往下補位的瀑布流。
...
疫情卡仍從既有 `delta` 資料推導「上週新增」、「較前三週平均」和「較前一週」，並與其他自動資料依序放在左側即時頻道。樂透卡保留兩行獎金與開獎日；一般 `delta` 卡列出各指標現值與相對變化；`compare`、`list`、`note` 則取既有提要、問題或首段作為摘要。
